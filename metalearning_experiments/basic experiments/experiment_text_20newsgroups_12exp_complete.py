...
categories = None  
seeds = [0, 1, 2]
search_iterations = [10, 30, 50, 100]

output_csv = "experiment_text_20newsgroups_12exps.csv"
...
print(f"Cargado 20 Newsgroups: {len(X_text)} documentos, con {len(target_names)} categorías")
...
n_classes = len(set(y))
domain = "text_20newsgroups_full"

print(f"Documentos: {n_samples}  -> TF-IDF shape: {X_matrix.shape},  #clases={n_classes}")

exclude_pattern = (
    "("
    "CountVectorizer|TfidfVectorizer|HashingVectorizer|"  # text transforms
    "KernelPCA|KernelCenterer|AdditiveChi2Sampler|Nystroem"  # NxN kernel transforms
    ")"
)
...
for seed in seeds:
    for iters in search_iterations:
        ...
        print(f"[20NG] seed={seed}, iters={iters} -> Score={best_score[0]:.4f}, Time={elapsed:.2f}s")
