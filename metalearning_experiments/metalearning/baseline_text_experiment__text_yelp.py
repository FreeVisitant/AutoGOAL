...
from autogoal.datasets import yelp_reviews
...
output_csv = "experiment_yelp_baseline.csv"
...
print("Cargando Yelp Reviews...")
# yelp_reviews.load(True) => (X_train, y_train, X_test, y_test)
...
for seed in seeds:
    for search_iters in search_iterations_list:
        ...
        best_score = automl.best_scores_[0]  # supuesta accuracy
        best_pipeline = automl.best_pipelines_[0]

        print(f"[Yelp] SEED={seed} | ITERS={search_iters} -> Score={best_score[0]:.4f} | Time={elapsed:.2f}s")
...
print("\nExperimento BASELINE Yelp finalizado.")
