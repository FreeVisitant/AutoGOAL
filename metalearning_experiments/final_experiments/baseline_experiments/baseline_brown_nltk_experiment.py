...
class ExperienceLoggerWithEvalTime(ExperienceLogger):
    def eval_solution(self, solution, fitness, observations):
        ...
        if hasattr(fitness, '__len__') and len(fitness) > 1:
            ...
            f1_score = fitness
            evaluation_time = 0.0  # Valor por defecto si no se devuelve el tiempo real
...
output_csv = "experiment_brown_baseline.csv"
...
documents = [(brown.raw(fid), brown.categories(fid)[0])
...
# Definir alias de dominio para Brown
domain = "brown"
...
print("\nExperimento BASELINE Brown finalizado.")
