...
try:
    ...
    logging.info("El corpus 'brown' ya está descargado.")
except LookupError:
    logging.info("Descargando el corpus 'brown' de NLTK...")
    nltk.download('brown')
...
# Archivo CSV para registrar resultados (dominio "brown")
output_csv = "experiment_brown_warm_start.csv"
...
print("Cargando corpus Brown...")


documents = [(brown.raw(fid), brown.categories(fid)[0]) for fid in brown.fileids()]
...
# Definir alias para la ejecución con warm start
domain = "brown_warm"  
...
warm_start = WarmStart(
    ...
    min_alpha=-0.02,
    beta_scale=1.0,
    normalizers=[],  # O agrega normalizadores si lo requieres
    ...
    exclude="^(brown_warm|brown)$",  # Excluye la experiencia actual y el baseline original
...
for seed in seeds:
    for search_iters in search_iterations_list:
        ...
        # Se crea el logger que registra el evaluation time
...
print("\nExperimento WARM_START Brown finalizado.")
